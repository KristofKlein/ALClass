...
Write-Host '###### Handle obsolete NAVContainerhelper! ######' -ForegroundColor Blue

$NAVmodule = Get-InstalledModule -Name navcontainerhelper -ErrorAction SilentlyContinue
if ($NAVmodule) {
    Write-Host "removing NavContainerHelper ..." 
    Uninstall-Module $NAVmodule -AllVersions -Force
}
else {
    Write-Host "Navcontainerhelper has been removed." 
}

Write-Host '###### Update BCContainerhelper! ######' -ForegroundColor Blue
